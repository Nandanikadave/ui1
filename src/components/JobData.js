    ...
    <>
      ...
      <div className="flex space-x-24 w-full border-b-2 px-24 py-7 ">
        <div className="flex flex-col space-y-2 items-start justify-center">
          <h1 className="text-gray-500 font-medium text-sm">Skills Required</h1>

          {detail.skillsRequired.map((skill) => (
            <div
              key={skill.Name}
              className="flex text-xs space-x-1 font-medium text-gray-700 justify-start items-center w-fit h-fit border-2 rounded-md"
            >
        ...
        <div className="flex flex-col items-start">
          ...
          <h1 className="text-gray-700 font-bold text-sm">{detail.Lan}</h1>
        </div>

        <div className="flex flex-col items-start">
          <h1 className="text-gray-500 font-medium text-sm">Type</h1>
          <h1 className="text-gray-700 font-bold text-sm">{detail.type}</h1>
        </div>
        ...
        <div className="flex flex-col items-start">
          ...
          <h1 className="text-gray-700 font-bold text-sm">{detail.exp}</h1>
        </div>
      </div>
      ...
      <div className="flex flex-col space-y-2 w-full border-b-2 px-24 py-10">
        ...
        <ol className="list-decimal pl-4 flex flex-col justify-start items-start">
          {detail.jobDetails.description.map((description) => (
            <li className="text-gray-600 text-base font-semibold ">
              {description}
            </li>
          ))}
        </ol>

        <h1 className="text-gray-600 text-base font-semibold">Benefits:</h1>
        <div>
          {detail.jobDetails.benefits.map((benefit) => (
            <ol className="text-gray-700 pl-10 relative flex items-center space-x-2">
              <li className=" list-disc text-gray-600 text-base font-semibold ">
                {benefit}
              </li>
            </ol>
          ))}
        ...
        <h1 className="text-gray-600 text-base font-semibold">Schedule:</h1>
        <div>
          <ol className="text-gray-700 pl-10 relative flex items-center space-x-2">
            <li className=" list-disc text-gray-600 text-base font-semibold ">
              {detail.jobDetails.schedule}
            </li>
          </ol>
        </div>
        ...
        <div>
          {detail.jobDetails.PayTypes.map((payType) => (
            <ol className="text-gray-700 pl-10 space-x-2 relative flex items-center space-x-2">
              <li className="list-disc text-gray-600 text-base font-semibold">
                {payType}
              </li>
            </ol>
        ...
        <div className="flex items-center space-x-2">
          <h1 className="text-gray-600 text-base font-semibold">Workplace:</h1>
          <span className="text-gray-600 text-base font-semibold">
            {detail.jobDetails.Location}
          </span>
        </div>
      </div>
    </>
